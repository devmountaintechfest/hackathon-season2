...
# [python] user query search by region
#
# v1.0.0 09/10/65
#   - Initial Release
#
...
static_savejson_filepath     = 'lastestresult_userquery_searchby_region.json'


# ================== Function ========================
# get user input
def GetUserInput():
    print("Script search employee by region")
    searchkeyword = input("Enter region name : ")
...
def SearchSQLiteDB(_searchfield="",_searchkeyword=""):
    ...
    #query databasecad
    myquery = "SELECT * " + \
        "FROM vEMPLOYEEBYREGION WHERE " + _searchfield + " = '" +_searchkeyword + "'"
    mycursor = conn.cursor()
    mycursor.execute(myquery)
    rows = mycursor.fetchall()
    if not rows:
        print("SQL result : Not found record with region name : " + _searchkeyword)
        jsonfile_clear()
        return False, None #apistatus, result_json
    temp_result_list = []
    for row in rows:
...
def main():
    ...
    apistatus, result_json = SearchSQLiteDB("REGION",searchkeyword)

    # STEP #3 PRINT RESULT AS JSON
    time_now = perf_counter()
    print("Elapsed running time : " + str(time_now-time_then) + " seconds")
    if apistatus:
        print("SQL Result : found record with region name : " + searchkeyword + " , total record : " + str(len(result_json)))
