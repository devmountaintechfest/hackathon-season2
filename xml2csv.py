...
def xml2csv_converter_helper(records, convert_default):
    ...
    for record in records:
        if convert_default and (
            ...
            or record[0].text == record[1].text
            or record[0].text in existed_EMPID_set
            or record[1].text in existed_PASSPORT
            or not ensure_years_experiences(record[7].text, years=3)
        ):
            continue
    ...
    with open(csv_file, "w", newline="") as output_file:
        ...
        for record in xml2csv_converter_helper(records, convert_default):
            ...
            if nationality not in csv_by_nationality:
                csv_by_nationality_file = (
                    f"{csv_file[:-4]}-{nationality}.csv"
                    if csv_file[-4:] == ".csv"
                    else csv_file
                )
                csv_by_nationality[nationality] = csv.writer(
                    open(csv_by_nationality_file, "w", newline=""), delimiter=","
                )
...
def main():
    try:
        ...
        xml2csv_converter(
            xml_file=xml_file, csv_file=csv_file, convert_default=convert_default
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Specify the name of a xml file to read from."
    )
    parser.add_argument(
        "--csv",
